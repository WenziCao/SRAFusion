...
from src.trainer.trainer_ISeg import train_one_epoch
...
def train_isg(cfg):
    ...
    vision_file_path = './visualization/runs/' + cfg.MODEL.SEG_NET.TYPEI
    create_file(vision_file_path)
    ...
    pg = [p for p in ir_seg_net.parameters() if p.requires_grad]
    ...
    for epoch in range(cfg.TRAINER.START_EPOCH, cfg.TRAINER.EPOCH+1):
        ...
        mean_loss = train_one_epoch(cfg, ir_seg_net, optimizer, train_loader, epoch, logger)
        logger.info('epoch %d train mean loss: %.4f \n' % (epoch, mean_loss))
        if epoch % cfg.TRAINER.SV_ER_EP == 0:
            checkpoint = {
                'model_state_dict': ir_seg_net.state_dict(),
