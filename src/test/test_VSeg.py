...
from src.utils.tool import create_file, visualize, compute_results
...
def test_vsg(cfg):
    ...
    # saved model path
    save_model_path = os.path.join('./model_hub/', cfg.MODEL.SEG_NET.TYPEV,
                                   'cp-epoch-{}.pt'.format(cfg.TEST_EPOCH))
    # visual image dir

    # get the model
    model = build_vsg_net(cfg)
    model.to(cfg.DEVICE)
    model.eval()
    ...
    with torch.no_grad():
        for it, (images_vis, images_ir, labels, name) in enumerate(test_loader):
            ...
            # save demo images
            fpath = []
            create_file('./visualization/VL_v_logit')
            for k in range(len(name)):
                fpath.append('./visualization/VL_v_logit/' + name[k])
